...
def stage_comparison_chart(
    ...
    *,
    ...
    time_col: str = "time",
    hf_col: str = "hf",
    stage_pct_col: str = "hf_pct",
    stage_rank_col: str = "div_factor_standing",
    use_stage_rank_as_metric: bool = False,
    ...
    Parameters
    ----------
    use_stage_rank_as_metric : bool
        If True, use `stage_rank_col` on the y-axis instead of `metric_col`.
    """
    if not shooters:
        st.info(select_message)
        return

    y_col = stage_rank_col if use_stage_rank_as_metric else metric_col
    y_label = "Stage Rank" if use_stage_rank_as_metric else metric_label

    required_cols = {shooter_col, stage_col, y_col}
    ...
    agg_map = {
        ...
        "pts": (pts_col, "mean") if pts_col in chart_df.columns else (y_col, "mean"),
        "time": (time_col, "mean") if time_col in chart_df.columns else (y_col, "mean"),
        "hf": (hf_col, "mean") if hf_col in chart_df.columns else (y_col, "mean"),
        "stage_pct": (stage_pct_col, "mean") if stage_pct_col in chart_df.columns else (y_col, "mean"),
        "stage_rank": (stage_rank_col, "min") if stage_rank_col in chart_df.columns else (y_col, "mean"),
    }
    ...
    if show_stage_average:
        stage_avg_df = (
            full_df.groupby(stage_col, as_index=False)[y_col]
            .mean()
            .rename(columns={y_col: "stage_avg"})
    ...
    for shooter in shooters_str:
        ...
        customdata = np.column_stack(
            [
                ...
                shooter_df["stage_pct"].to_numpy(dtype=float),
            ]
        )
